# GamblingApp
A modular Python backend application for simulating gambling sessions, strategies, and win/loss calculations.
This project demonstrates 7 use cases (UC1–UC7) including gambler profile management, stake tracking, betting strategies, session management, odds-based games, input validation, and interactive UI.

Features / Use Cases
Gambler Profile Management – Create, view, update, reset profiles.

Stake Management – Place bets, view stake history reports.

Strategy Betting – Fixed, Percentage, Martingale, Reverse Martingale, Fibonacci, D’Alembert.

Session Management – Start, play, pause, resume, end sessions with summaries.

Win/Loss Calculation – Play games with different odds types and outcome strategies.

Input Validation – Safe input handling with configurable validation rules.

User Interaction – Interactive menu, game status display, and session summary UI.
